...
The model becomes:
$$ \log{(crmrte)} = -2.542 -0.0147 \cdot prbarr -0.253 \cdot \log{(prbconv)} + 0.422 \cdot \log{(density)} + 0.000151 \cdot pctmin80 $$
...
$$ \log{(crmrte)} = \beta_0 + \beta_1\ prbarr + \beta_2\ \log{(prbconv)} + \beta_3\ \log{(density)} + \beta_4\ {pctmin80} + u $$
...
When we examined the residuals versus fitted values plot, it was apparent that the variance of errors in the middle of the plot is higher than the variance of errors in the edges of the plot. This suggested heteroskedasticity, so we examined
...
Despite this violation of CLM, we are able to proceed with our OLS model by using heteroskedasticity-robust standard errors.
...
We continue to see $pctymle$ as not significant.  In addition, $log_taxpc$ and $wfed$ are also not statistically significant.  $polpc$ is significant at p=0.05.
