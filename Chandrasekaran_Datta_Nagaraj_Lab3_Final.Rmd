...
We now present some influential outliers we found during our analysis that affect our regression fit.

In general, the outliers are counties that are heavy on tourism, but with low resident population.  This causes high crime rates and police presence even in the presence of low density, because density probably counts only residents.

$wser$, observation #84: extremely high service wages. This observation is so skewed that it pulls the regression line of fit.  The county (FIPS: 185) corresponds to Warren; there is nothing in Wikipedia to suggest that this county can command such high service wages.
...
$polpc$, observation #51: highly influential observation, which has very low crime rate, yet very high police per capita.  This observation corresponds to Madison County in the mountains of western NC.  This county gets a lot of tourists, so it may have more police than otherwise.  This merits further investigation.  We will remove it to improve our fit.
...
$density$, observation #78: extremely low density.  Looking at FIPS numbering for the county (173), we see that this corresponds to Swain County, western North Carolina.  This county is almost entirely forested and an influential outlier.  It also derives revenue from tourism.  We can remove this county to improve our fit.
...
Finally, observation #25 causes outliers in the final model fit due to a lot of influence. It shows very high
crime rate and very high police per capita, while also showing very low density, very low minority and highest tax per capita.

The observation corresponds to Dare County, which consists mostly of a sliver of an island.  NCPedia lists seasonal tourism as the primary industry of the county.  This can explain why there is high crime and police presence, even though there is low population density and high tax revenue.

This observation has a high Cook's distance (>1).  It also is an unusual, outlier county; we removed it to improve our model fit.
...
```{r}
crime_data %>% filter(crmrte > 0.07) %>% dplyr::select(density, central, urban, prbarr, prbconv)
```

We see that outlier high-crime areas are in central, urban North Carolina.
...
Tax revenue also shows positive skew, with one outlier indicating high tax revenue per capita (>100).  It does not show a lot of leverage (Cook's distance > 1), however, so we will keep the value.
...
The model becomes:
$$ \log{(crmrte)} = -2.744 -0.0138 \cdot prbarr -0.236 \cdot \log{(prbconv)} + 0.420 \cdot \log{(density)} + 0.000150 \cdot pctmin80 $$

**Practical significance:**

From the above regression equation, we can infer the following.  All statements have *ceteris paribus* assumptions:

* If we hold all other variables constant, we have a "base crime rate" of $e^{-2.744} = 0.0643$.

* For a unit increase in "probability" of arrest, crime rate goes down by approximately $|e^{-0.0138} - 1| = 0.0138$, or 1.37%.  This is a large effect.  It shows that law enforcement is a very good deterrent to crime.

* For a 1% increase in "probability" of conviction, crime rate decreases by about 0.3%.  This is not a lot.  It may be because most of the effect is already absorbed by $prbarr$.

* For a 1% increase in density, crime rate increases by about 0.4%.  This is a large effect.  As we put more people closer, crime is easier to commit and harder to spot.

* For a 1% increase in minorities, crime rate increases by about $|e^{0.000150 - 1}| = 0.015\%$.  This is a minuscule effect.  Compared to the animosity the majority may have over minorities, the effect is very little.
...
## CLM on Model 2
Here we will pick our second model and do a thorough analysis of six assumptions of the linear model namely:

#### CLM 1.0 - Linear in parameters
Our model is represented as:

$$\log{(crmrte)} = \beta_0 + \beta_1\ prbarr + \beta_2\ \log{(prbconv)} + \beta_3\ \log{(density)} + \\
                  \beta_4\ {pctmin80} + \beta_6\ {pctymle} + \beta_7\ \log{(polpc)} + \beta_8\ \log{(taxpc)} 
                  + \beta_9\ {(wfed)} + u $$
...
We do note that variables such as crimerate are prone to clustering effects but we can get away with bootstrapping measurements
...
mean is met. The red spline curve does not deviate much from zero and appears fairly flat
```{r}
plot(model2, which = 1)
```
...
Despite this violation of CLM, we are able to proceed with our OLS model by using heteroskedasticity-robust
standard errors.
```{r}
plot(model2, which = 3)
bptest(model2)
...
Looking at the histogram of the residual term indicates a more or less normal distribution. This can further be confimed by the Shapiro test. The null hypothesis of this test is that residuals have a normal distribution. The p-value indicates we do not reject this null hypothesis.
...
For both our other models (1 and 3), we observed that most of the CLM assumptions were satisfied. They both supported: linear in parameters, random sampling, multicollinearity and had normal residuals. 
The one difference was that these other models actually satisfied Homoskedasticity as well. We believe that by adding all the variables, as in model3, we are likely canceling out variances in errors 
