...
```{r}
crime_data_sql_central=sqldf('SELECT * FROM crime_data WHERE central=1 AND 
                         (west!=1 AND urban!=1)')
crime_data_sql_west=sqldf('SELECT * FROM crime_data WHERE west=1 AND 
                      (central!=1 AND urban!=1)')
```
...
```{r}
par(mfrow=c(2,4), mai=c(0.35,0.35,0.35,0.35))
boxplot(crime_data_sql_central$crmrte, main = "Crime: Central", ylim=c(0.0,0.1))
boxplot(crime_data_west$crmrte, main = "Crime: West", ylim=c(0.0,0.1))
hist(crime_data_sql_central$crmrte, main="Crime: Central", breaks=15, 
     xlim=c(0.0, 0.1), ylim=c(0, 7))
hist(crime_data_sql_west$crmrte, main="Crime: West", breaks=15, 
     xlim=c(0.0, 0.1), ylim=c(0, 7))
```
...
We now see that $pctymle$ is not significant.  In addition, $log_taxpc$ and $wfed$ are also not statistically significant.  $polpc$ is significant at p=0.05.
