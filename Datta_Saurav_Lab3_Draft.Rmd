...
For model 4, which is for region west, we get a r-squared value of 0.93, which is significantly better than the all region model, Model2.

## Model 4b
We will now apply Model2s for region Central.
...
The r-squared value of 0.83 is still better than that of Model2.


Comparing the Cook's distance plot of the region specific models, we see that none of the observations in the region specific models have  a Cook's distance greater than 1, unlike that in the region-generic model
...
2) Median total household income
...
5) Climate when the crime occured
...
So, if a model uses prbarr, prbconv, prbpris, avgsen, the OVB bias would be negative.

The median total household income in the county may have a negative correlation to crime rate. This OV will also have a negative correlation with prbarr, prbconv, prbpris, avgsen, and  a positive correlation with taxpc.
So, if a model uses prbarr, prbconv, prbpris, avgsen, the OVB bias would be negative. For a model using taxpc, the OVB bias would be positive.

The Total number of neighbour crime watch groups in the county may have a negative correlation to crime rate. This OV will also have a negative correlation with prbarr, prbconv, prbpris, avgsen.
So, if a model uses prbarr, prbconv, prbpris, avgsen, the OVB bias would be negative.
...
#o_variable_names will appear as row headers
ov_names=c("Median level of education","Employment")

ov_bias_effect=c("A higher median level of education in the county may have a negative correlation to crime","Employment may may have a negative correlation to crime")

ov=matrix(ov_bias_effect,ncol=1,byrow=TRUE)
colnames(ov)=c("Bias result")
rownames(ov) = ov_names
ov
