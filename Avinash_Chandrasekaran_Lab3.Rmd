...
Observation #25 also causes outliers in the final model fit due to a lot of influence.  It shows very high crime rate, very high police per capita, very low density, highest tax per capita, very low minority.  It is an observation with too many quirks and may merit separate investigation.

Here are all the outlier observations:

```{r echo=FALSE}
outlier_data = crime_data %>% slice(c(25, 51, 79, 84))
str(outlier_data, give.attr=FALSE, give.head=FALSE)
```

The second observation above has very low crime rate at very high police per capita.  The third has extremely low density.  The fourth has extremely high wages in the service industry.
...
```{r}
crime_data <- crime_data %>% slice(-c(25, 51, 79, 84))
```
...
Let us explore $wcon$ as a proxy for all wages.
...
* We believe that the following cause higher crime: high density, higher percentage of minorities, higher percentage of young men.
...
The fit improves to 0.808 (adjusted $R^2$).  The wage $wfed$ is not significant.  $taxpc$ is not significant either.

We will now build a third model that includes almost all the variables, for the sake of comparison.  We only exclude wages because their distribution is highly alike.
...
# 7. Discussion of omitted variables (Identify what you think are the 5-10 most important omitted variables that bias results you care about.)
