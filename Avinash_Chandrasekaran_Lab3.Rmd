...
The plot looks fairly normal, but there are 2 outliers.  It may seem odd that the value goes above 1.0 for a probability, but this is not a true probability: this is a ratio of arrests to offenses.  It may be that the convict escaped and a second arrest was made.  The outlier at $prbarr=1.1$ has a Cook's distance of 51, with considerable leverage.  Nevertheless, we will keep the values for now.

There is fairly strong negative correlation of -0.39: as probability of arrests increases, crime rate goes down.  It may be that arrests are a deterrent.
...
There is moderate negative correlation with crime rate of -0.39.  As convictions go up, crime rate goes down.  Since we have already considered $prbarr$, let us check if $prbconv$ has high correlation with $prbarr$:
...
```{r}
f_describe_col(crime_data$prbpris, plot_model=TRUE)
