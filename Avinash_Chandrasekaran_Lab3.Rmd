...
f_describe_col = function (col, do_log=FALSE, plot_model=FALSE, do_sqrt=FALSE) {
  ...
  m = lm(y ~ x)
  plot(x, y, main="Cor. with crime rate", col=c("green", "red"),
       xlab="predictor (green)", ylab="log_crmrte (red)")
  if (is.numeric(col))
    abline(m, col="blue")
  if (plot_model == TRUE) {
    if (do_log == TRUE) {
...
Let us explore $wfed$ as a proxy variable for all wages, for a second model we will propose.

```{r}
f_describe_col(crime_data$wfed, plot_model=TRUE)
```
