...
f_describe_col = function (col, do_log=FALSE) {
  print(summary(col))
  par(mfrow=c(2,2))
  hist(col, main="Histogram")
  boxplot(col, main="Box plot")
  m = lm(crime_data$crmrte ~ col)
  plot(col, crime_data$crmrte, main="Cor. with crime rate")
  abline(m, col="blue")
  print(signif(cor(col, crime_data$crmrte), 3))
  if (do_log==TRUE) {
    hist(log(col), main="Histogram, log")
    log_col = paste("log_", col)
    crime_data$log_col = log(col)
  }
}
```
