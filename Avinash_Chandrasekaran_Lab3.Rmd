...
Our first step in EDA is to detect anomalies such as missing & duplicate values and attempt to clean up the dataset before deeper dive into regression analysis
...
We found 6 rows of NA's across all variables. Due to this We simply omit these NA's.
...
Further, we noticed that 'prbconv' is a factor while the rest of the variables are numeric. We convert 'prbconv' to match other numeric variables in our data set.
...
There are a total of 90 observations across 25 different variables. We will now explore each of the variables collected in the data.
### Crime rate
...
We don't see much correlation and will include *log_prbconv* in our model.
...
### Police per capita
Police per capita has positive skew.  And we observed that taking the log tranformation made the distribution more normal.
...
We also observe a high correlation betweeb "urban" and "density", so we run a risk of multicollinearity.
...
## TODO: Remove diagrams, keep correlation.
...
# TODO: Check correlation matrix one more time.
# cor(prbarr, polpc) is positive: check again.
...
# TODO: Remove qqplot from model2, model3?
...
The fit improves to 0.808 (adjusted $R^2$).  Although we added it, the wage $wfed$ is not significant.  $taxpc$ is not significant either. 
