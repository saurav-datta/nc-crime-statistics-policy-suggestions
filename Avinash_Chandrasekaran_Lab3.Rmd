...
f_describe_col = function (col, do_log=FALSE, plot_model=FALSE) {
  print(summary(col))
  par(mfrow=c(2,2))
  ...
  print(signif(cor(x, crime_data$crmrte), 3))
  m = lm(crime_data$crmrte ~ x)
  plot(x, crime_data$crmrte, main="Cor. with crime rate")
  abline(m, col="blue")
  if (plot_model == TRUE) {
    plot(m)
    if (do_log == TRUE) {
      m = lm(crime_data$crmrte ~ log(col))
      plot(m)
    }
  }
}
...
We will start with police per capita.
...
The distribution looks better now.  We see quite strong positive correlation of 0.55 with crime rate: high number of police per capita is associated with high crime rate.  It is probably a cause, rather than a result.  More police may have been deployed to deal with higher amount of crime.
...
The plot looks fairly normal, but there are 2 outliers.  It may seem odd that the value goes above 1.0 for a probability, but this is not a true probability: this is a ratio of arrests to offenses.  It may be that the convict escaped and a second arrest was made.  Nevertheless, we will keep the values for now.

There is fairly strong negative correlation of -0.38: as probability of arrests increases, crime rate goes down.  It may be that arrests are a deterrent.
...
This histogram plot looks fairly normal and we don't observe any weird values.  However, correlation is almost nonexistent wrt crime rate.
...
Perhaps we can make a policy recommendation to consider reducing sentences over long periods of time, or to be more lenient in pardoning criminals already serving long sentences.
...
The histogram of density shows a bit positive skew. The log transformation shows a more promising normal distribution.  There are no outliers with large leverage as measured by Cook's distance.

There is only very slight positive correlation with crime rate.  We will *not* consider this variable for a first model.
